...
get_commands_for_language() {
    ...
    if [[ -n "$lang" ]]; then
        echo "# Fill project-specific commands for $lang\\n- [TEST_COMMAND]\\n- [LINT_COMMAND]\\n- [BUILD_COMMAND]"
    else
        echo "# Fill project-specific commands\\n- [TEST_COMMAND]\\n- [LINT_COMMAND]\\n- [BUILD_COMMAND]"
    fi
}
...
get_language_conventions() {
    local lang="$1"
    if [[ -n "$lang" ]]; then
        echo "$lang: Follow project-defined conventions"
    else
        echo "[LANGUAGE_CONVENTIONS]: Follow project-defined conventions"
    fi
}
